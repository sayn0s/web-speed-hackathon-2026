...
export const ImageArea = ({ images, eager }: Props) => {
  return (
      ...
      <div className="border-cax-border grid h-full w-full grid-cols-2 grid-rows-2 gap-1 overflow-hidden rounded-lg border">
        {images.map((image, idx) => {
          return (
            ...
            >
              <CoveredImage alt={image.alt} src={getImagePath(image.id)} loading={eager ? "eager" : "lazy"} fetchPriority={eager ? "high" : undefined} />
            </div>
          );
        })}
      </div>
